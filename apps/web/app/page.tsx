...
const interests = ['All', 'Conversation', 'Design', 'Startups', 'Science', 'Wellbeing', 'Culture', 'Technology'];
const learningLevels = [
  { name: 'Beginner', range: 'A1–A2', note: 'Clear, everyday language' },
  { name: 'Intermediate', range: 'B1–B2', note: 'Natural speech with support' },
  { name: 'Advanced', range: 'C1–C2', note: 'Nuanced, fast-paced ideas' },
];

function searchableText(podcast: Podcast) {
  return [podcast.title, podcast.author, podcast.topic, podcast.episode, podcast.description, podcast.level, podcast.cefr, ...podcast.tags]
    .join(' ')
    .toLowerCase();
}
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return podcasts.filter((podcast) => {
      const matchesInterest = activeInterest === 'All' || podcast.topic === activeInterest;
      const matchesLevel = podcast.level === activeLevel;
      const matchesQuery = !normalized || searchableText(podcast).includes(normalized);
      return matchesInterest && matchesLevel && matchesQuery;
    });
  }, [activeInterest, activeLevel, query]);
  ...
  useEffect(() => {
    ...
    register({
      ...
      execute(input) {
        ...
        const nextLevel = typeof level === 'string' ? level : 'Beginner';
        const matches = podcasts.filter((podcast) => podcast.level === nextLevel && searchableText(podcast).includes(nextQuery.toLowerCase()));
        setActiveInterest('All');
        setActiveLevel(nextLevel);
        setQuery(nextQuery);
        setShowAll(true);
        return { query: nextQuery, level: nextLevel, resultCount: matches.length, titles: matches.map((podcast) => podcast.title) };
      },
    });
    ...
    register({
      ...
      inputSchema: {
        type: 'object',
        properties: { podcastId: { type: 'integer', minimum: 1 } },
        required: ['podcastId'],
        additionalProperties: false,
      },
      annotations: { readOnlyHint: false, untrustedContentHint: false },
      execute(input) {
        ...
        if (typeof id !== 'number' || !podcasts.some((podcast) => podcast.id === id)) throw new Error('podcastId was not found');
        setSaved((current) => (current.includes(id) ? current : [...current, id]));
        const podcast = podcasts.find((item) => item.id === id)!;
        return { saved: true, podcastId: id, title: podcast.title };
      },
    });
        ...
        <section className="mt-12" aria-labelledby="results-heading">
          ...
          {visible.length ? (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {visible.map((podcast, index) => {
                const isPlaying = playing === podcast.id;
                const isSaved = saved.includes(podcast.id);
                return (
                  <article className="group rounded-[26px] border border-border bg-card p-3 shadow-[0_14px_40px_rgba(48,42,35,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_55px_rgba(48,42,35,0.1)]" key={podcast.id}>
                    <div className={`${podcast.tone} ${podcast.ink} relative aspect-[4/3] overflow-hidden rounded-[18px] p-5`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2"><span className="text-xs font-semibold uppercase tracking-[0.12em] opacity-60">{podcast.topic}</span>{podcast.verifiedLesson && <span className="rounded-full bg-white/55 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.08em]">Real lesson</span>}</div>
                        <button aria-label={`${isSaved ? 'Remove' : 'Save'} ${podcast.title}`} aria-pressed={isSaved} className="grid size-9 place-items-center rounded-full bg-white/35 backdrop-blur-sm transition hover:bg-white/55" onClick={() => toggleSaved(podcast.id)} type="button">
                          {isSaved ? <Check className="size-4" /> : <Bookmark className="size-4" />}
                        </button>
                      </div>
                      <p className="mt-4 max-w-[9ch] font-serif text-[clamp(1.7rem,3vw,2.5rem)] leading-[0.93] tracking-[-0.05em]">{podcast.title}</p>
                      <span className="absolute bottom-4 left-4 rounded-full bg-white/60 px-3 py-1.5 text-xs font-semibold backdrop-blur-sm">{podcast.level} · {podcast.cefr}</span>
                      <Button aria-label={`${isPlaying ? 'Pause' : 'Play'} ${podcast.title}`} onClick={() => setPlaying(isPlaying ? null : podcast.id)} className="absolute bottom-4 right-4 size-12 rounded-full bg-[#24211f] text-white hover:bg-[#24211f]/85" size="icon">
                    ...
                    <div className="px-2 pb-2 pt-4">
                      <div className="flex items-start justify-between gap-3">
                        <div><h3 className="font-semibold tracking-[-0.02em]">{podcast.title}</h3><p className="mt-0.5 text-sm text-muted-foreground">{podcast.author}</p></div>
                        <span className="shrink-0 rounded-full bg-secondary px-2.5 py-1 text-xs font-medium text-secondary-foreground">{podcast.match}% fit</span>
                      </div>
                      <p className="mt-4 text-sm font-medium leading-5">{podcast.episode}</p>
                      <p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{podcast.description}</p>
                      <div className="mt-4 rounded-xl bg-secondary/55 p-3">
                        ...
                        <p className="mt-1.5 text-xs leading-5 text-muted-foreground">{podcast.levelReason}</p>
                        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-border/70 pt-3 text-xs text-muted-foreground">
                          <span className="flex items-center gap-1.5"><Gauge className="size-3.5" />{podcast.speed}</span>
                          <span className="flex items-center gap-1.5"><BookOpenText className="size-3.5" />{podcast.newWords} new words</span>
                          <span className="flex items-center gap-1.5"><Clock3 className="size-3.5" />{podcast.duration}</span>
                        </div>
                      </div>
                      <a className="mt-3 flex h-10 items-center justify-center gap-2 rounded-full border border-border text-sm font-semibold transition hover:border-foreground/25 hover:bg-secondary" href={`/episode/${podcast.id}`}>Start lesson<ArrowRight className="size-4" /></a>
                    </div>
                  </article>
                );
              })}
