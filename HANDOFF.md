...
## What "remove from my learning list" removes (2026-09-05)

The list page shows **bookmarks ∪ episodes with a listening position**, and the X button
used to clear only the first half. So removing anything the learner had actually opened
did nothing visible: the row stayed, and the button silently turned into a Bookmark icon,
which reads as a broken delete. That was the reported bug.

Removal now clears both — the bookmark, and the listening position (`currentTime` back to
0, `complete` back to false) via `writeListeningPosition` in `apps/web/lib/learner-store.ts`.

**What it deliberately does not touch: `savedWords`, `answers`, `checked`.** They live in
the same `discopod-progress-*` record as the position, so deleting the record would have
been the one-line fix and would have silently emptied the vocabulary page. A position is
regenerated by pressing play; a saved word is not. The undo bar says so in words —
*"Any words you saved from it are still in your word list"* — rather than leaving the
learner to find out.

**A stored record is not the same as "listened to".** Saving a word off the transcript
writes a progress record without the audio ever moving, and a removed episode keeps its
words. So membership is decided by `hasPosition()` (`currentTime > 0 || complete`), not by
the record existing. Without that, a removed episode would reappear on the next render,
and an episode you only ever picked a word from would appear having never been played.

Removing is reversible for one action (`Undo` restores the bookmark *and* the position it
had), which is why there is no confirmation dialog. The sample list has no X at all —
